...
CharacterSelectEvent.OnClientEvent:Connect(function(logos)
    ...
    for i, character in Characters do
        if i == logo_id then
            print("[Turkey Hub] Opponent's character is "..character.name.." ("..tostring(i)..")")
        ...
        if v then
            task.spawn(function()
                while AutoplayEnabled do
                    local prompt = workspace.DuelTables.Table_16.Seat1:FindFirstChild("SitPrompt")
                    if prompt then
                        fireproximityprompt(prompt)
                        task.wait(0.5)
                        RequestOpponent:FireServer(16)
                        task.wait(0.5)
